...
import ViewDetails from '../../OrdersPage/Components/ViewDetails';
...
function CompletedOrders({ completedOrders, formatTime }) {
    ...
    return (
        <>
            {completedOrders.map(order => {
                return (
                    <div key={order.orderId} className="bg-white border border-zinc-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-green-300 transition-colors shadow-sm">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 flex-1">
                            ...
                            <div>
                                ...
                                <p className={`text-xs font-bold ${order.paymentStatus === 'Paid' ? 'text-green-600' : 'text-green-500'}`}>
